...
BMP* b_deep_fake_copy(BMP* to_copy,int width,int height,unsigned char** red,unsigned char** green,unsigned char** blue)
{
    BMP* copy = (BMP*) malloc(sizeof(BMP));
    copy->file_byte_number = to_copy->file_byte_number-(3*(to_copy->width*to_copy->height))+(3*height);
    //printf("Diferença de tamanhos: %d e %d\n",copy->file_byte_number,to_copy->file_byte_number);
    copy->pixel_array_start = to_copy->pixel_array_start;

    int new_width=255;
    int new_height=(height*width)/255;
    if(new_height%255!=0){
        new_height++;
    }

    copy->width = new_width;
    copy->height = new_height;
    copy->depth = to_copy->depth;
    //printf("DEEP FAKE: %d e %d\n",copy->width,copy->height);
    ...
    for (y = 0; y < copy->height; y++)      
    {
        for (x = 0; x < copy->width; x++)
        {
            //printf("%d:PIXEIS: %d e %d\n",line,y,x);
            if(line<height){
                index = y * copy->width + x;
                copy->pixels[index].red = red[line][0];
                copy->pixels[index].green = green[line][0];
                copy->pixels[index].blue = blue[line][0];
                copy->pixels[index].alpha = to_copy->pixels[index].alpha;   //FIXME criar vetor de alphas
                line++;
            }
        }
    }
    ...
    unsigned int i;
    /*for (i = 0; i < copy->pixel_array_start+12; i++)
    {
        printf("%d:Copia %d\n",i,to_copy->file_byte_contents[i]);
    }*/
    for (i = 0; i < copy->pixel_array_start; i++)
    {
        copy->file_byte_contents[i] = to_copy->file_byte_contents[i];
    }
    copy->file_byte_contents[18]=copy->width;
    copy->file_byte_contents[22]=copy->height;

    int channels = copy->depth / (sizeof(unsigned char) * BITS_PER_BYTE);
    int row_size = ((int) (copy->depth * copy->width + 31) / 32) * 4;
    int padding = row_size - copy->width * channels;
    int c;
    unsigned int offset;
    for (y = 0; y < copy->height; y++)
    {
        ...
        {
            index = y * copy->width + x;
            offset = copy->pixel_array_start + index * channels + y * padding;
            for (c = 0; c < channels; c++)
            {
                ...
                {
                    case BLUE:
                        value = copy->pixels[index].blue;
                        break;
                    case GREEN:
                        value = copy->pixels[index].green;
                        break;
                    case RED:
                        value = copy->pixels[index].red;
                        break;
                    case ALPHA:
                        value = copy->pixels[index].alpha;
                        break;
                }
                copy->file_byte_contents[offset + c] = value;
            }
        }
    }
...
void _map(BMP* bmp, void (*f)(BMP*, int, int, int))
{
    ...
    for (y = 0; y < bmp->height; y++)
    {
        for (x = 0; x < bmp->width; x++)
        {
            ...
            //if((y==0 && x==0) || (y==270 && x==0) || (y==3000 && x==0))
                //printf("\n%d||%d:MAP mete index %d e offset %d\n",y,x,index,offset);
            
            for (c = 0; c < channels; c++)
            {
                //printf("MAP Entra com index %d e %d offset e c %d\n",index,offset,c);
